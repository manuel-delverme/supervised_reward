...
class MiniGridEnv(gym.Env):
    ...
    def get_view_exts(self):
        ...
        # Facing right
        if config.trivial_observations:
    ...
    def step(self, action):
        ...
        # Toggle/activate an object
            ...
            if fwd_cell:
                # if fwd_cell.type == 'door' and fwd_cell.is_locked and config.shape_reward:
                #     reward += 1
    ...
    def gen_obs_grid(self):
        ...
        if not config.trivial_observations:
    ...
    def render(self, mode='human', close=False, **kwargs):
        ...
        if self.grid_render is None:
            ...
            self.grid_render = Renderer(
                self.width * CELL_PIXELS + config.agent_view_size * CELL_PIXELS,
        ...
        if r.window:
            ...
            if 'action_value' in kwargs:
                av = kwargs['action_value']
                del kwargs['action_value']
                av = '\n'.join(av)
            else:
                av = ""
        ...
        for vis_col in range(0, self.agent_view_size):
            for vis_row in range(0, self.agent_view_size):
                # If this cell is not visible, don't highlight it
                if not vis_mask[vis_row, vis_col]:
                    continue

                # Compute the world coordinates of this cell
                abs_row, abs_column = top_left - (f_vec * vis_col) + (r_vec * vis_row)

                # Highlight the cell
                r.fillRect(
                    abs_row * CELL_PIXELS,
                    abs_column * CELL_PIXELS,
                    CELL_PIXELS,
                    CELL_PIXELS,
                    255, 255, 255, 75
                )
        ...
        if isinstance(o, np.ndarray):
            if not config.trivial_observations:
                o = np.rot90(o.copy(), -(self.agent_dir + 1), axes=(1, 2))
            for layer in range(o.shape[0]):
                for vis_col in range(self.agent_view_size):
                    for vis_row in range(self.agent_view_size):
                        # Compute the world coordinates of this cell
                        abs_row = vis_row + layer * (self.agent_view_size + 1)
                        abs_column = self.width + vis_col

                        oi = o[layer, vis_row, vis_col]

                        colour = (oi * 255, oi * 255, 64, 255)
                        # if vis_row % self.agent_view_size == 0:
                        #     colour = (oi * 255, oi * 255, 255, 255)
                        # Highlight the cell
                        r.fillRect(
                            abs_column * CELL_PIXELS,
                            abs_row * CELL_PIXELS,
                ...
                #     y = CELL_PIXELS * config.agent_view_size
                #     r.drawLine(0, y, (self.width + config.agent_view_size) * CELL_PIXELS, y)
        r.endFrame()
