...
class QLearning(object):
    def __init__(self, env, options=None, epsilon=0.1, gamma=0.99, alpha=0.2, surrogate_reward=None):
        self.epsilon = epsilon
        self.gamma = gamma
        self.alpha = alpha

        self.Q = np.zeros((env.observation_space.n, env.action_space.n))
        # self.Q = 0.00001 * np.random.rand(env.observation_space.n, env.action_space.n)
    ...
    def pick_action(self, old_state):
        ...
        while primitive_action == TERMINATE_OPTION:
            if self.epsilon < random.random():
                best_action = np.argmax(self.Q[old_state, :])
    ...
    def learn(self, steps_of_no_change=100, generate_options=False):
        cumulative_reward = 0
        ...
        while no_change < steps_of_no_change:
            for position_idx in range(self.environment.number_of_tiles):
                ...
                if is_option(action):
                    ...
                    old_choice = primitive_action  # np.argmax(self.Q[old_state])
                    ...
                    self.Q[old_state][action] += self.alpha * delta_Q

                    new_choice = np.argmax(self.Q[old_state])
                    ...
                    if generate_options and delta_Q > 1:
                        ...
                        tmp_Q[:, :-1] = self.Q
                        self.Q = tmp_Q
                        self.Q[:, -1] = self.Q.mean(axis=1)
                    old_state = new_state
        self.environment.print_board(some_matrix=np.max(self.Q, axis=1), policy=np.argmax(self.Q, axis=1))
        time.sleep(5)
